...
def main():
    if len(sys.argv) != 3:
        print("Uzycie: python3 plot_zad4_0.py <plik_danych> <plik_wykresu>")
        sys.exit(1)
    ...
    plt.plot(E, Rn, label="R_numerow(E)")
    plt.xlabel("E")
    plt.ylabel("R_n (r = L)")
    plt.title("R_n = f(E) dla l = 1 (R0=0)")
    plt.grid(True)
    plt.legend()

    L = 1.0
    zeros_bessel_l1 = {3.8317, 7.0155, 10.1734, 13.3236};

    energies = [0.5 * (a / L) ** 2 for a in zeros_bessel_l1]
